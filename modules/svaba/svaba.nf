process SVABA {
  ...
  publishDir "${params.pubdir}/${ params.organize_by=='sample' ? "$sampleID" : 'svaba' }", pattern: "*.log", mode:'copy'
  publishDir "${params.pubdir}/${ params.organize_by=='sample' ? "$sampleID" : 'svaba' }", pattern: "*.vcf.gz", mode:'copy'
