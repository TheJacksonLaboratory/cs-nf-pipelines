#!/usr/bin/env python
"""
Parse SV caller VCFs to extract IDs and depth information
...
def parse_vcf(vcf_file, out_file):
    ...
    ----------
    vcf_file: str
        The path to the VCF file from an SV caller
    ...
    with open(out_file, 'w', newline='\n') as ofile:
        ...
        writer.writerow(['SV_ID', 'DR', 'DV'])
        vcf_reader = vcf.Reader(filename=vcf_file)
        for record in vcf_reader:
            writer.writerow([record.ID, 
                             str(record.samples[0]['DR']).replace('[','').replace(']','').replace(', ',';'),
                             str(record.samples[0]['DV']).replace('[','').replace(']','').replace(', ',';')])
