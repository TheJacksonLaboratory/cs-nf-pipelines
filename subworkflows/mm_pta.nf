...
include {SVTYPER} from "${projectDir}/modules/svtyper/svtyper"
include {LUMPY_COMPRESS_INDEX} from "${projectDir}/modules/utility_modules/lumpy_compress_index"
...
workflow MM_PTA {
    ...
    main:
        ...
        */
        ...
        // ** Lumpy - SV Calling
        LUMPY_SV(ch_paired_samples)
        SVTYPER(LUMPY_SV.out.lumpy_sv_vcf)
        LUMPY_COMPRESS_INDEX(SVTYPER.out.typed_vcf)
        ...
        Lumpy
        LUMPY_COMPRESS_INDEX.out.vcf_tbi
        ...
        */
        ...
        // tuple val(sampleID), val(normal_name), val(tumor_name), file(manta_vcf), file(manta_vcf_tbi), val(meta_manta), val(manta), file(lumpy_vcf), val(lumpy_tbi), val(meta_lumpy), val(lumpy)
        ...
        merge_sv_input = MANTA.out.manta_somaticsv_tbi.join(LUMPY_COMPRESS_INDEX.out.vcf_tbi, by : [0,4,5])
        ...
        // tuple val(sampleID), val(normal_name), val(tumor_name), path(manta_vcf), path(manta_vcf_tbi), path(lumpy_vcf), path(lumpy_tbi), path(delly_vcf), path(delly_tbi)
